        ...
        <div>
          ...
          <div className="flex flex-col items-end gap-[13px] pr-2">
            {weekRooms?.map((item, index) => {
              return (
                <div className="relative flex h-[32px] w-[187px] items-center justify-center rounded-18 bg-white cursor-pointer" key={item.roomId} onClick={() => handleWeekClick(item.roomId)}>
                  <p className="text-sm">{item.week}주차</p>
                  <div className="absolute -left-[10%] -top-1/2 translate-y-1/3">
                    ...
                    />
                    <div
                      className={`absolute left-0 top-0 w-[3.74px] bg-white ${index === weekRooms.length - 1 ? 'h-[10px]' : 'h-[53px]'}`}
                    />
                  </div>
                </div>
              )
