...
const RoomCreationForm = ({ onSubmit }: any) => {
  ...
  const renderSchedule = () => {
    return userClassData?.map((item: any) => (
      <div key={item.couseCode} className="mb-2 p-2 border-b flex flex-col" onClick={() => route.push(`/chat/${item.courseCode}`)}>
        <div className="font-bold">{item.courseName} ({item.courseCode})</div>
        <div>{item.room}</div>
